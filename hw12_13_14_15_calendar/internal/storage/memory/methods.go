...
// Создать событие
func (r *MemRepo) CreateEvent(ctx context.Context, event *st.EventModel) (id int64, err error) {
	r.LastId++
	id = r.LastId
	event.ID = r.LastId
	r.Repo[id] = *event
...
// Обновить событие;
func (r *MemRepo) UpdateEvent(ctx context.Context, event *st.EventModel) error {
	if event.ID == 0 {
...
// Удалить событие;
func (r *MemRepo) DelEvent(ctx context.Context, id int64) error {
	delete(r.Repo, id)
	return nil
}
...
func (r *MemRepo) getEventsForTimeInterval(ctx context.Context, start, end time.Time) ([]st.EventModel, error) {
