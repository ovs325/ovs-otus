...
func TestBucketOk(t *testing.T) {
	...
	assert.Equal(t, time.Second-time.Nanosecond, bucket.DeadLine(), msgErrDeadL) // Проверить Промежуток времени до опустошения корзины
	...
	assert.Equal(t, time.Nanosecond, bucket.DeadLine(), msgErrDeadL)             // Проверить Промежуток времени до опустошения корзины
	sr.SetElapsed(time.Second)                                                   // Установить сдвиг времени
	assert.Equal(t, time.Second, sr.GetElapsed(), msgErrSetElp)                  //
	assert.Equal(t, time.Duration(0), bucket.DeadLine(), msgErrDeadL)            // Проверить Промежуток времени до опустошения корзины
	...
	assert.Equal(t, time.Second/2, bucket.DeadLine(), msgErrDeadL)                // Проверить Промежуток времени до опустошения корзины
	realAddDrops, ok = bucket.AddDrops(drops)                                     // колличество реально добавленных капель
	assert.True(t, ok)                                                            //
	assert.Equal(t, drops, realAddDrops)                                          //
	...
	capacity = bucket.GetCapacity()                                               //
	assert.Equal(t, time.Second, bucket.DeadLine(), msgErrDeadL)                  // Проверить Промежуток времени до опустошения корзины
	bucket.ChangeCapacity(int64(4))                                               //
	capacity = bucket.GetCapacity()                                               //
	assert.Equal(t, time.Second, bucket.DeadLine(), msgErrDeadL)                  // Проверить Промежуток времени до опустошения корзины
	...
	assert.Equal(t, int64(0), bucket.DropsSum(), msgErrDropS)                     //
	assert.Equal(t, time.Duration(0), bucket.DeadLine(), msgErrDeadL)             // Проверить Промежуток времени до опустошения корзины
	drops = int64(5)                                                              //
	realAddDrops, ok = bucket.AddDrops(drops)                                     //
	assert.True(t, ok)                                                            //
	...
	assert.Equal(t, time.Second*4, bucket.DeadLine(), msgErrDeadL)                // Проверить Промежуток времени до опустошения корзины
