...
var WhiteCmd = &cb.Command{
	Use:   "white",
	Short: "Добавление Сети/Подсети в белый лист",
	Long:  "Команда добавление Сети/Подсети в белый список",
	...
	RunE: func(cmd *cb.Command, args []string) error {
		...
		url, err := url.Parse(fmt.Sprintf("%s://%s:%s/add/white", pr.ConfigHttp.Scheme, pr.ConfigHttp.Host, pr.ConfigHttp.Port))
		if err != nil {
